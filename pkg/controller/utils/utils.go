...
func getPodIPs(cl client.Client, kind string, namespace string, name string) ([]string, error) {
	var podIPs []string

	switch kind {
	case "pod":
		pod := &corev1.Pod{}
		err := cl.Get(context.TODO(), types.NamespacedName{Name: name, Namespace: namespace}, pod)
		if err != nil {
			return podIPs, fmt.Errorf("pod (namespace: %s, name: %s) not found", namespace, name)
		}
		podIPs = append(podIPs, pod.Status.PodIP)
		break
	//TODO: Impelent other resource cases
	case "rc":
	case "ds":
	default:
		return podIPs, fmt.Errorf("%s is not supported kind", kind)
	}
